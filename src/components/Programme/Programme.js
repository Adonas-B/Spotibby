...
export class Programme extends Component {
  ...
  handleAddClick = () => {
    ...
    fetch(
      `${process.env.REACT_APP_API_URL}/programme/${this.props.programme.programme_id}/add?access_token=${access_token}`
    )
      .then((res) => {
        if (res.status !== 200) {
          console.log(
  ...
  handleClick = () => {
    if (this.state.tracks) {
      ...
      this.setState({ isLoading: true });
      fetch(
        `${process.env.REACT_APP_API_URL}/programme/${this.props.programme.programme_id}`
      )
        .then((res) => {
          if (res.status !== 200) {
            console.log(
  ...
  render() {
    ...
    if (tracks) {
      tracks_display = tracks.map((t) => (
        <Track artist={t.artist} title={t.title}></Track>
      ));
    }
